...
const updateServiceWithDeleteImgForMainSlider = async (Request, DataModel) => {
  ...
  try {
    ...
    if (deleteImg.result === "ok") {
      var pullImg = await DataModel.findOneAndUpdate(
        ...
        { $pull: { mainSlider: { public_id: imgId } } }
      );
...
module.exports = updateServiceWithDeleteImgForMainSlider;
